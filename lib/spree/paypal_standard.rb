module Spree::PaypalStandard
  ...
  # Outbound redirect to Paypoint via a form
  #
  def paypal_standard_payment
    debugme=1
  ...
  def paypal_standard_gateway
    payment_method.provider
  end

  def redirect_to_paypal_standard_form
    return true unless @order.state=="confirm" and @order.payment_method.is_a?(BillingIntegration::PaypalStandard)
    @order.payment.started_processing
    redirect_to paypal_standard_payment_order_checkout_url(@order, :payment_method_id => @order.payment_method.id)
  end
end
